...
// lineBuffer is an abstraction of an UCD document source.
...
func newLineBuffer(inputDoc io.Reader) *lineBuffer {
	...
	// From the spec:
	// Line breaks: A UCD document is partitioned into lines where the lines are split by
...
func (buf *lineBuffer) readRune() (rune, error) {
	...
	if readerErr != nil {
		return 0, fmt.Errorf("I/O error (%w) while reading input character", readerErr)
	}
	...
	for buf.isEof == 0 {
		...
		if !buf.Input.Scan() { // could not read a new line: either I/O-error or EOF
			if err := buf.Input.Err(); err != nil {
				return fmt.Errorf("I/O error (%w) while reading input", err)
			}
			//fmt.Println("===> EOF !")
			buf.isEof = 1
...
// This is a frequent operation for UCD items.
